...
> 🚀 **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Cline MCP Marketplace](https://github.com/bluesaicoder/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.bluesaicoderrules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Cline/MCP).

### Using `.bluesaicoderrules` Files

A `.bluesaicoderrules` file is a special configuration that Cline reads automatically when working in the directory where it's placed. These files:
...
Here's the complete MCP Server Development Protocol that should be placed in your `.bluesaicoderrules` file:
...
When this `.bluesaicoderrules` file is present in your working directory, Cline will:
...
### 1. Create a `.bluesaicoderrules` file (🚨 IMPORTANT)

First, add a `.bluesaicoderrules` file to the root of your MCP working directory using the protocol above. This file configures Cline to use the MCP development protocol when working in this folder.
...
<Frame>
	<img
		src="https://storage.googleapis.com/bluesaicoder_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/bluesaicoder_public_images/docs/assets/building-mcp-plugin.gif"
