...
1. **The `new_task` Tool:**
    ...
    - **Context Preloading:** Crucially, Cline can **preload** this new task session with specific context provided within the tool's `<context>` block. This context can be anything Cline or a `.bluesaicoderrules` file defines – summaries, code snippets, next steps, project state, etc.
...
#### Default Behavior (Without `.bluesaicoderrules`)

By default, without specific `.bluesaicoderrules` dictating its behavior:
...
#### The Power of `.bluesaicoderrules`: Enabling Custom Workflows

While the core capabilities exist by default, the true power, automation, and customization emerge when you combine `new_task` and context awareness with custom workflows defined in `.bluesaicoderrules`. This allows you to precisely control _when_ and _how_ Cline manages context and task continuity.

Key benefits of using `.bluesaicoderrules` with `new_task`:
...
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.bluesaicoderrules` to have Cline automatically create a new task via `new_task` upon completing each subtask, preloading the context for the _next_ subtask.
-   **Custom Context Packaging:** Mandate the exact structure and content of the `<context>` block in `.bluesaicoderrules` for highly detailed and consistent handoffs (see example below).
...
A common workflow, **driven by specific `.bluesaicoderrules` like the example below**, involves these steps:
...
4. **Context Packaging (`new_task` Tool):** If approved, Cline uses `new_task`, packaging the context according to the structure mandated by the `.bluesaicoderrules`.
...
The effectiveness of rule-driven handoffs depends heavily on how `.bluesaicoderrules` define the `<context>` block. A comprehensive structure often includes:
...
The flexibility of `new_task` combined with `.bluesaicoderrules` opens up many possibilities:
...
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.bluesaicoderrules` to have Cline automatically create a new task via `new_task` upon completing each subtask.
...
Experimenting with `.bluesaicoderrules` is encouraged to discover workflows that best suit your needs!

#### Example `.bluesaicoderrules`: Task Handoff Strategy Guide

Below is an example `.bluesaicoderrules` file focused specifically on using `new_task` for context window management. **Remember, this is just one specific strategy; the core `new_task` tool can be used differently with other custom rules.**
...
-   **Flexibility:** The core `new_task` tool is a flexible building block. Experiment with `.bluesaicoderrules` to create workflows that best suit your needs, whether for strict context management, task decomposition, or other creative uses.
```
````
