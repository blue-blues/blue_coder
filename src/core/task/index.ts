...
import { BluesAICoderHandler } from "@api/providers/blues-ai-coder"
...
import {
	BluesAICoderApiReqCancelReason,
	BluesAICoderApiReqInfo,
	BluesAICoderAsk,
	BluesAICoderMessage,
	BluesAICoderSay,
...
import { BluesAICoderAskResponse, BluesAICoderCheckpointRestore } from "@shared/WebviewMessage"
...
import { BluesAICoderErrorType } from "@services/error/BluesAICoderError"
...
import {
	getGlobalBluesAICoderRules,
	getLocalBluesAICoderRules,
	refreshBluesAICoderRulesToggles,
} from "@core/context/instructions/user-instructions/blues-ai-coder-rules"
import {
	getLocalCursorRules,
...
import { BluesAICoderIgnoreController } from "@core/ignore/BluesAICoderIgnoreController"
...
import { convertBluesAICoderMessageToProto } from "@shared/proto-conversions/blues-ai-coder-message"
import { isClaude4ModelFamily, isGemini2dot5ModelFamily } from "@utils/model-utils"
import { isInTestMode } from "../../services/test/TestMode"
import { ensureLocalBluesAICoderDirExists } from "@core/context/instructions/user-instructions/rule-helpers"
import { refreshWorkflowToggles } from "../context/instructions/user-instructions/workflows"
import { SequentialAgent } from "@core/agent/SequentialAgent"
...
export class Task {
	...
	private bluesAICoderIgnoreController: BluesAICoderIgnoreController
	...
	constructor(
		...
		this.bluesAICoderIgnoreController = new BluesAICoderIgnoreController(cwd)
		...
		let effectiveApiConfiguration: ApiConfiguration = {
			...apiConfiguration,
			bluesAiCoderAccountId: apiConfiguration.bluesAiCoderAccountId,
			taskId: this.taskId,
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
				const lastApiReqStartedIndex = findLastIndex(bluesaicoderMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: BluesAICoderApiReqInfo = JSON.parse(
							bluesaicoderMessages[lastApiReqStartedIndex].text || "{}",
						)
						...
						await this.messageStateHandler.updateBluesAICoderMessage(lastApiReqStartedIndex, {
	...
	async restoreCheckpoint(messageTs: number, restoreType: BluesAICoderCheckpointRestore, offset?: number) {
		const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
		const messageIndex = bluesaicoderMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
		// Find the last message before messageIndex that has a lastCheckpointHash
		const lastHashIndex = findLastIndex(
			bluesaicoderMessages.slice(0, messageIndex),
			(m) => m.lastCheckpointHash !== undefined,
		)
		const message = bluesaicoderMessages[messageIndex]
		const lastMessageWithHash = bluesaicoderMessages[lastHashIndex]

		if (!message) {
			console.error("Message not found", bluesaicoderMessages)
			return
		}
		...
		if (!didWorkspaceRestoreFail) {
			switch (restoreType) {
				...
				case "taskAndWorkspace":
					...
					const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
					const deletedMessages = bluesaicoderMessages.slice(messageIndex + 1)
					...
					const newBluesAICoderMessages = bluesaicoderMessages.slice(0, messageIndex + 1)
					await this.messageStateHandler.overwriteBluesAICoderMessages(newBluesAICoderMessages) // calls saveBluesAICoderMessages which saves historyItem
			...
			await this.messageStateHandler.saveBluesAICoderMessagesAndUpdateHistory()
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean) {
		...
		const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
		const messageIndex = bluesaicoderMessages.findIndex((m) => m.ts === messageTs)
		const message = bluesaicoderMessages[messageIndex]
		if (!message) {
		...
		try {
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.messageStateHandler.getBluesAICoderMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges() {
		...
		const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
		const messageIndex = findLastIndex(bluesaicoderMessages, (m) => m.say === "completion_result")
		const message = bluesaicoderMessages[messageIndex]
		if (!message) {
	...
	async ask(
		type: BluesAICoderAsk,
		text?: string,
		partial?: boolean,
	): Promise<{
		response: BluesAICoderAskResponse
		text?: string
		images?: string[]
		files?: string[]
	}> {
		...
		if (partial !== undefined) {
			const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
			const lastMessage = bluesaicoderMessages.at(-1)
			const lastMessageIndex = bluesaicoderMessages.length - 1
			const isUpdatingPreviousPartial =
				lastMessage && lastMessage.partial && lastMessage.type === "ask" && lastMessage.ask === type
			if (partial) {
				if (isUpdatingPreviousPartial) {
					// existing partial message, so update it
					await this.messageStateHandler.updateBluesAICoderMessage(lastMessageIndex, {
						text,
						partial,
					})
					...
					// await this.postStateToWebview()
					const protoMessage = convertBluesAICoderMessageToProto(lastMessage)
					...
					await this.messageStateHandler.addToBluesAICoderMessages({
						ts: askTs,
						type: "ask",
						ask: type,
						text,
	...
	async handleWebviewAskResponse(askResponse: BluesAICoderAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async say(type: BluesAICoderSay, text?: string, images?: string[], files?: string[], partial?: boolean): Promise<undefined> {
		if (this.taskState.abort) {
			throw new Error("BluesAICoder instance aborted")
		}

		if (partial !== undefined) {
			const lastMessage = this.messageStateHandler.getBluesAICoderMessages().at(-1)
			const isUpdatingPreviousPartial =
	...
	async sayAndCreateMissingParamError(toolName: ToolUseName, paramName: string, relPath?: string) {
		await this.say(
			"error",
			`BluesAICoder tried to use ${toolName}${
				relPath ? ` for '${relPath.replace(/\\/g, "/")}'` : ""
			} without value for required parameter '${paramName}'. Retrying...`,
		)
		return formatResponse.toolError(formatResponse.missingToolParameterError(paramName))
	}

	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: BluesAICoderAsk | BluesAICoderSay) {
		const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
		const lastMessage = bluesaicoderMessages.at(-1)
		if (lastMessage?.partial && lastMessage.type === type && (lastMessage.ask === askOrSay || lastMessage.say === askOrSay)) {
			this.messageStateHandler.setBluesAICoderMessages(bluesaicoderMessages.slice(0, -1))
			await this.messageStateHandler.saveBluesAICoderMessagesAndUpdateHistory()
		}
	}
	...
	private async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		try {
			await this.bluesAICoderIgnoreController.initialize()
		} catch (error) {
			console.error("Failed to initialize BluesAICoderIgnoreController:", error)
			// Optionally, inform the user or handle the error appropriately
		}
		// conversationHistory (for API) and bluesaicoderMessages (for webview) need to be in sync
		// if the extension process were killed, then on restart the bluesaicoderMessages might not be empty, so we need to set it to [] when we create a new BluesAICoder client (otherwise webview would show stale messages from previous session)
	...
	private async resumeTaskFromHistory() {
		...
		const savedBluesAICoderMessages = await getSavedBluesAICoderMessages(this.getContext(), this.taskId)
		...
		const lastBluesAICoderMessage = this.messageStateHandler
			.getBluesAICoderMessages()
			.slice()
			.reverse()
			.find((m) => !(m.ask === "resume_task" || m.ask === "resume_completed_task")) // could be multiple resume tasks

		let askType: BluesAICoderAsk
		if (lastBluesAICoderMessage?.ask === "completion_result") {
		...
		const agoText = (() => {
			const timestamp = lastBluesAICoderMessage?.ts ?? Date.now()
			const now = Date.now()
		...
		const wasRecent = lastBluesAICoderMessage?.ts && Date.now() - lastBluesAICoderMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		if (
			...
			userContent[0].text.includes("<task>\nrun sequential agent\n</task>")
		) {
			const sequentialAgent = new SequentialAgent(this.toolExecutor, this.taskState, this.messageStateHandler)
			await sequentialAgent.run()
			await this.say("text", "Sequential agent has finished its run.")
			return
		}
	...
	async abortTask() {
		...
		this.bluesAICoderIgnoreController.dispose()
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false) {
		...
		this.messageStateHandler.getBluesAICoderMessages().forEach((message) => {
			...
			if (this.checkpointTracker) {
				...
				if (commitHash) {
					...
					const lastCheckpointMessageIndex = findLastIndex(
						this.messageStateHandler.getBluesAICoderMessages(),
						(m) => m.say === "checkpoint_created",
					)
					if (lastCheckpointMessageIndex !== -1) {
						await this.messageStateHandler.updateBluesAICoderMessage(lastCheckpointMessageIndex, {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const { globalToggles, localToggles } = await refreshBluesAICoderRulesToggles(this.getContext(), this.cwd)
		...
		const globalBluesAICoderRulesFileInstructions = await getGlobalBluesAICoderRules(
			globalBluesAICoderRulesFilePath,
			globalToggles,
		)

		const localBluesAICoderRulesFileInstructions = await getLocalBluesAICoderRules(this.cwd, localToggles)
		...
		const bluesaicoderIgnoreContent = this.bluesAICoderIgnoreController.bluesaicoderIgnoreContent
		let bluesaicoderIgnoreInstructions: string | undefined
		if (bluesaicoderIgnoreContent) {
			bluesaicoderIgnoreInstructions = formatResponse.bluesaicoderIgnoreInstructions(bluesaicoderIgnoreContent)
		}

		if (
			globalBluesAICoderRulesFileInstructions ||
			localBluesAICoderRulesFileInstructions ||
			localCursorRulesFileInstructions ||
			localCursorRulesDirInstructions ||
			localWindsurfRulesFileInstructions ||
			bluesaicoderIgnoreInstructions ||
		...
		try {
			...
			const isOpenRouter = this.api instanceof OpenRouterHandler || this.api instanceof BluesAICoderHandler
			...
			const bluesaicoderError = ErrorService.toBluesAICoderError(error, modelId, providerId)

			// Capture provider failure telemetry using bluesaicoderError
			// TODO: Move into ErrorService
			telemetryService.captureProviderApiError({
				taskId: this.taskId,
				model: modelInfo.id,
				errorMessage: bluesaicoderError.message,
				errorStatus: bluesaicoderError._error?.status,
				requestId: bluesaicoderError._error?.request_id,
			})
			...
			if (isAnthropic && isAnthropicContextWindowError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				const streamingFailedMessage = bluesaicoderError.serialize()
				...
				if (bluesaicoderError.isErrorType(BluesAICoderErrorType.Auth)) {
					return
				}
	...
	async recursivelyMakeBluesAICoderRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		...
		const isFirstRequest =
			this.messageStateHandler.getBluesAICoderMessages().filter((m) => m.say === "api_req_started").length === 0
		...
		if (bluesaicoderulesError === true) {
			await this.say(
				"error",
				"Issue with processing the /newrule command. Double check that, if '.bluesaicoderules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory.",
			)
		}
		...
		await this.messageStateHandler.updateBluesAICoderMessage(lastApiReqIndex, {
			text: JSON.stringify({
				request: userContent.map((block) => formatContentBlockToMarkdown(block)).join("\n\n"),
			} satisfies BluesAICoderApiReqInfo),
		})
		...
		try {
			...
			const abortStream = async (cancelReason: BluesAICoderApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			try {
				...
				if (!this.taskState.abandoned) {
					...
					const errorMessage = bluesaicoderError.serialize()
			...
			if (assistantMessage.length > 0) {
				...
				const recDidEndLoop = await this.recursivelyMakeBluesAICoderRequests(this.taskState.userMessageContent)
	...
	async loadContext(userContent: UserContent, includeFileDetails: boolean = false): Promise<[UserContent, string, boolean]> {
				...
				userContent.map(async (block) => {
					if (block.type === "text") {
						...
						if (
							...
							if (needsCheck) {
								needsBluesAICoderulesFileCheck = true
							}
		...
		// Return all results
		return [processedUserContent, environmentDetails, bluesaicoderulesError]
	}
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		// Filter paths through bluesaicoderIgnoreController
		const allowedVisibleFiles = this.bluesAICoderIgnoreController
		...
		// Filter paths through bluesaicoderIgnoreController
		const allowedOpenTabs = this.bluesAICoderIgnoreController
		...
		*/
		...
		if (includeFileDetails) {
			details += `\n\n# Current Working Directory (${this.cwd.replace(/\\/g, "/")}) Files\n`
			...
			if (isDesktop) {
				...
				const result = formatResponse.formatFilesList(this.cwd, files, didHitLimit, this.bluesAICoderIgnoreController)
				details += result
			}
		...
		const getTotalTokensFromApiReqMessage = (msg: BluesAICoderMessage) => {
			if (!msg.text) {
				return 0
			}
			try {
		...
		const bluesaicoderMessages = this.messageStateHandler.getBluesAICoderMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(bluesaicoderMessages.slice(1)))
