...
import {
	...
	browserActions,
	BluesAICoderAsk,
	BluesAICoderAskQuestion,
	BluesAICoderAskUseMcpServer,
	BluesAICoderPlanModeResponse,
	BluesAICoderSay,
	BluesAICoderSayBrowserAction,
	BluesAICoderSayTool,
	COMPLETION_RESULT_CHANGES_FLAG,
} from "@shared/ExtensionMessage"
import { BluesAICoderAskResponse } from "@shared/WebviewMessage"
...
import { fileManipulationToolDefinition, fileManipulationToolName } from "../tools/fileManipulationTool"
...
export class ToolExecutor {
	...
	private askApproval = async (type: BluesAICoderAsk, block: ToolUse, partialMessage: string) => {
	...
	public async executeTool(block: ToolUse): Promise<void> {
		...
		switch (block.name) {
			case fileManipulationToolName: {
				const { operation, path: relPath, content } = block.params
				if (!operation || !relPath) {
					break
				}

				const accessAllowed = this.bluesAICoderIgnoreController.validateAccess(relPath)
				if (!accessAllowed) {
					await this.say("blues_ai_coder_ignore_error", relPath)
					this.pushToolResult(formatResponse.toolError(formatResponse.bluesaicoderIgnoreError(relPath)), block)
					await this.saveCheckpoint()
					break
				}
				...
				const sharedMessageProps: BluesAICoderSayTool = {
					tool: "readFile", // Placeholder, will be updated based on operation
				...
				if (!(await this.shouldAutoApproveToolWithPath(block.name, relPath))) {
					showNotificationForApprovalIfAutoApprovalEnabled(
						`BluesAICoder wants to perform a '${operation}' operation on ${path.basename(relPath)}`,
					...
					switch (operation) {
						...
						case "write": {
							if (content === undefined || content === null) {
								this.pushToolResult(formatResponse.toolError("Content is missing for write operation."), block)
								break
							}
							...
							this.pushToolResult(formatResponse.fileEditWithoutUserChanges(relPath, undefined, content, ""), block)
							break
						}
						case "delete": {
							if (!fileExists) {
								this.pushToolResult(formatResponse.toolError(`File not found at path: ${relPath}`), block)
								break
							}
							await vscode.workspace.fs.delete(vscode.Uri.file(absolutePath))
							this.workspaceTracker.populateFilePaths()
							this.pushToolResult(`Successfully deleted file: ${relPath}`, block)
							break
						}
					}
				} catch (error) {
					await this.handleError(`performing '${operation}' operation`, error, block)
				}
				await this.saveCheckpoint()
				break
			}
				...
				try {
					...
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, relPath)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to read ${path.basename(absolutePath)}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to view directory ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to view source code definitions in ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
				...
				try {
					if (block.partial) {
						...
						if (await this.shouldAutoApproveToolWithPath(block.name, block.params.path)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to search files in ${path.basename(absolutePath)}/`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
					...
					if (block.partial) {
						if (action === "launch") {
							...
							await this.say(
								...
								} satisfies BluesAICoderSayBrowserAction),
								undefined,
								undefined,
								block.partial,
							)
						...
						if (action === "launch") {
							...
							if (this.shouldAutoApproveTool(block.name)) {
								...
								showNotificationForApprovalIfAutoApprovalEnabled(
									`BluesAICoder wants to use a browser and launch ${url}`,
									this.autoApprovalSettings.enabled,
									this.autoApprovalSettings.enableNotifications,
								)
								this.removeLastPartialMessageIfExistsWithType("say", "browser_action_launch")
			...
			case "execute_command": {
				...
				try {
					if (block.partial) {
						...
						const ignoredFileAttemptedToAccess = this.bluesAICoderIgnoreController.validateCommand(command)
						if (ignoredFileAttemptedToAccess) {
							await this.say("blues_ai_coder_ignore_error", ignoredFileAttemptedToAccess)
							this.pushToolResult(
								formatResponse.toolError(formatResponse.bluesaicoderIgnoreError(ignoredFileAttemptedToAccess)),
								block,
							)
							await this.saveCheckpoint()
							break
						...
						if (
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to execute a command: ${command}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
			...
			case "use_mcp_tool": {
				...
				try {
					if (block.partial) {
						...
						} satisfies BluesAICoderAskUseMcpServer)
						...
						if (mcp_arguments) {
							try {
								...
								await this.say(
									"error",
									`BluesAICoder tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
								)
								this.pushToolResult(
						...
						if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to use ${tool_name} on ${server_name}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
				...
				try {
					if (block.partial) {
						...
						if (this.shouldAutoApproveTool(block.name)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to access ${uri} on ${server_name}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
			...
			case "ask_followup_question": {
				...
				} satisfies BluesAICoderAskQuestion
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "BluesAICoder has a question...",
			...
			case "new_task": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "BluesAICoder wants to start a new task...",
								message: `BluesAICoder is suggesting to start a new task with: ${context}`,
							})
						}
			...
			case "condense": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "BluesAICoder wants to condense the conversation...",
								message: `BluesAICoder is suggesting to condense your conversation with: ${context}`,
							})
						}
			...
			case "report_bug": {
				...
				try {
					if (block.partial) {
						...
						if (this.autoApprovalSettings.enabled && this.autoApprovalSettings.enableNotifications) {
							showSystemNotification({
								subtitle: "BluesAICoder wants to create a github issue...",
								message: `BluesAICoder is suggesting to create a github issue with the title: ${title}`,
							})
						}

						// Derive system information values algorithmically
						const operatingSystem = os.platform() + " " + os.release()
						const bluesaicoderVersion =
							vscode.extensions.getExtension("saoudrizwan.bluesaicoder")?.packageJSON.version || "Unknown"
						...
						const bugReportData = JSON.stringify({
							...
							bluesaicoder_version: bluesaicoderVersion,
						})
						...
						if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
							...
							try {
								...
								params.set("bluesaicoder-version", bluesaicoderVersion)
								...
								await createAndOpenGitHubIssue("bluesaicoder", "bluesaicoder", "bug_report.yml", params)
							} catch (error) {
				...
				try {
					if (block.partial) {
						...
						if (this.shouldAutoApproveTool("web_fetch" as ToolUseName)) {
							...
							showNotificationForApprovalIfAutoApprovalEnabled(
								`BluesAICoder wants to fetch content from ${url}`,
								this.autoApprovalSettings.enabled,
								this.autoApprovalSettings.enableNotifications,
							)
							this.removeLastPartialMessageIfExistsWithType("say", "tool")
			...
			case "plan_mode_respond": {
				...
				const sharedMessage = {
					...
					options: parsePartialArrayString(this.removeClosingTag(block, "options", optionsRaw)),
				} satisfies BluesAICoderPlanModeResponse
				try {
					if (block.partial) {
			...
			case "attempt_completion": {
				...
				const addNewChangesFlagToLastCompletionResultMessage = async () => {
					...
					if (
						...
						await this.messageStateHandler.updateBluesAICoderMessage(lastCompletionResultMessageIndex, {
