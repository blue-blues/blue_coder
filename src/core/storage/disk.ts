...
export const GlobalFileNames = {
	...
	mcpSettings: "bluesaicoder_mcp_settings.json",
	bluesaicoderRules: ".bluesaicoderules",
	workflows: ".bluesaicoderules/workflows",
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const bluesaicoderRulesDir = path.join(userDocumentsPath, "BluesAICoder", "Rules")
	try {
		await fs.mkdir(bluesaicoderRulesDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "BluesAICoder", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return bluesaicoderRulesDir
}

export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const bluesaicoderWorkflowsDir = path.join(userDocumentsPath, "BluesAICoder", "Workflows")
	try {
		await fs.mkdir(bluesaicoderWorkflowsDir, { recursive: true })
	} catch (error) {
		return path.join(os.homedir(), "Documents", "BluesAICoder", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return bluesaicoderWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "BluesAICoder", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (error) {
		return "~/Documents/BluesAICoder/MCP" // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function getSavedBluesAICoderMessages(context: vscode.ExtensionContext, taskId: string): Promise<BluesAICoderMessage[]> {
...
export async function saveBluesAICoderMessages(context: vscode.ExtensionContext, taskId: string, uiMessages: BluesAICoderMessage[]) {
	try {
