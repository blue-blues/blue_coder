...
export class Controller {
	...
	mcpHub: McpHub
	accountService: BluesAICoderAccountService
	...
	constructor(
		...
		this.id = id
		this.outputChannel.appendLine("BluesAICoderProvider instantiated")
		...
		this.accountService = BluesAICoderAccountService.getInstance()
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			HostProvider.window.showMessage({
				...
				message: "Successfully logged out of Blues AI Coder",
			})
		} catch (error) {
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async handleAuthCallback(customToken: string, provider: string | null = null) {
		try {
			...
			const bluesProvider: ApiProvider = "blues-ai-coder"
			...
			if (planActSeparateModelsSetting) {
				...
				if (currentMode === "plan") {
					await updateGlobalState(this.context, "planModeApiProvider", bluesProvider)
				} else {
					await updateGlobalState(this.context, "actModeApiProvider", bluesProvider)
				}
			} else {
				// Update both modes to keep them in sync
				await Promise.all([
					updateGlobalState(this.context, "planModeApiProvider", bluesProvider),
					updateGlobalState(this.context, "actModeApiProvider", bluesProvider),
				])
			}
			...
			HostProvider.window.showMessage({
				...
				message: "Failed to log in to Blues AI Coder",
			})
	...
	private async fetchMcpMarketplaceFromApi(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get(`https://mcp.blues.ai/marketplace`, {
				headers: {
	...
	private async fetchMcpMarketplaceFromApiRPC(silent: boolean = false): Promise<McpMarketplaceCatalog | undefined> {
		try {
			const response = await axios.get(`https://mcp.blues.ai/marketplace`, {
				headers: {
					"Content-Type": "application/json",
					"User-Agent": "blues-ai-coder-vscode-extension",
				},
			})
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const localBluesAiCoderRulesToggles =
			((await getWorkspaceState(this.context, "localBluesAICoderRulesToggles")) as BluesAICoderRulesToggles) || {}

		const localWindsurfRulesToggles =
			((await getWorkspaceState(this.context, "localWindsurfRulesToggles")) as BluesAICoderRulesToggles) || {}

		const localCursorRulesToggles =
			((await getWorkspaceState(this.context, "localCursorRulesToggles")) as BluesAICoderRulesToggles) || {}

		const localWorkflowToggles =
			((await getWorkspaceState(this.context, "workflowToggles")) as BluesAICoderRulesToggles) || {}
		...
		return {
			...
			bluesAiCoderMessages: this.task?.messageStateHandler.getBluesAICoderMessages() || [],
			...
			globalBluesAICoderRulesToggles: globalBluesAICoderRulesToggles || {},
			localBluesAICoderRulesToggles: localBluesAiCoderRulesToggles || {},
