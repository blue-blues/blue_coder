...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "bluesaicoder-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .bluesaicoderrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const bluesaicoderrulesDirTest = path.join(tmpDir, "bluesaicoderrules-test")
		const bluesaicoderrulesDirPath = path.join(bluesaicoderrulesDirTest, ".bluesaicoderrules")

		// Create .bluesaicoderrules directory and root files
		await fs.mkdir(bluesaicoderrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(bluesaicoderrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(bluesaicoderrulesDirPath, "settings.js"), "// settings")

		// Create .bluesaicoderrules/other directory and files
		const otherDirPath = path.join(bluesaicoderrulesDirPath, "other")
		...
		// Create .bluesaicoderrules/workflows directory and files
		const workflowsDirPath = path.join(bluesaicoderrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(bluesaicoderrulesDirPath)
		...
		const filteredFiles = await readDirectory(bluesaicoderrulesDirPath, [[".bluesaicoderrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(bluesaicoderrulesDirPath, "config.json"),
			path.resolve(bluesaicoderrulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(bluesaicoderrulesDirPath, [
			[".bluesaicoderrules", "workflows"],
			[".bluesaicoderrules", "other"],
		])
