...
export class McpHub {
  ...
  "mcpServers": {
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { type: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or BluesAICoder (and we want to detect save events, not every file change)
