...
import { BluesAICoderAccountUserInfo, BluesAICoderAuthInfo } from "../AuthService"
...
export class FirebaseAuthProvider {
	 ...
	 */
	async retrieveBluesAICoderAuthInfo(context: ExtensionContext): Promise<BluesAICoderAuthInfo | null> {
		const userRefreshToken = await getSecret(context, "bluesAiCoderAccountId")
		if (!userRefreshToken) {
		...
		try {
			...
			// Store user data
			const userInfo: BluesAICoderAccountUserInfo = userResponse.data.data
	 ...
	 */
	async signIn(context: ExtensionContext, token: string, provider: string): Promise<BluesAICoderAuthInfo | null> {
		try {
			let credential
			switch (provider) {
				case "google":
			...
			try {
				await storeSecret(context, "bluesAiCoderAccountId", userCredential.refreshToken)
			} catch (error) {
			...
			return await this.retrieveBluesAICoderAuthInfo(context)
		} catch (error) {
