...
const DefaultBluesAICoderAccountURI = `${bluesAICoderEnvConfig.appBaseUrl}/auth`
...
export interface BluesAICoderAuthInfo {
	idToken: string
	userInfo: BluesAICoderAccountUserInfo
}

export interface BluesAICoderAccountUserInfo {
	createdAt: string
	displayName: string
	email: string
	id: string
	organizations: BluesAICoderAccountOrganization[]
	/**
	 * BluesAICoder app base URL, used for webview UI and other client-side operations
	 */
	appBaseUrl?: string
}

export interface BluesAICoderAccountOrganization {
	active: boolean
	memberId: string
	name: string
...
export class AuthService {
	...
	private _bluesaicoderAuthInfo: BluesAICoderAuthInfo | null = null
	...
	private constructor(context: vscode.ExtensionContext, config: ServiceConfig, authProvider?: any) {
		...
		this._config = Object.assign({ URI: DefaultBluesAICoderAccountURI }, config)
		...
		const authProvidersConfigs = [
			{
				name: "firebase",
				config: bluesAICoderEnvConfig.firebase,
			},
		]
	...
	async getAuthToken(): Promise<string | null> {
		if (!this._bluesaicoderAuthInfo) {
			return null
		}
		const idToken = this._bluesaicoderAuthInfo.idToken
		const shouldRefreshIdToken = await this._provider?.provider.shouldRefreshIdToken(idToken)
		if (shouldRefreshIdToken) {
			// Retrieves the stored id token and refreshes it, then updates this._bluesaicoderAuthInfo
			await this.restoreRefreshTokenAndRetrieveAuthInfo()
			if (!this._bluesaicoderAuthInfo) {
				return null
			}
		}
		return this._bluesaicoderAuthInfo.idToken
	}
	...
	getInfo(): AuthState {
		...
		let user: any = null
		if (this._bluesaicoderAuthInfo && this._authenticated) {
			const userInfo = this._bluesaicoderAuthInfo.userInfo
			this._bluesaicoderAuthInfo.userInfo.appBaseUrl = bluesAICoderEnvConfig?.appBaseUrl
	...
	async handleDeauth(): Promise<void> {
		...
		try {
			this._bluesaicoderAuthInfo = null
	...
	async handleAuthCallback(token: string, provider: string): Promise<void> {
		...
		try {
			this._bluesaicoderAuthInfo = await this._provider.provider.signIn(this._context, token, provider)
	...
	async clearAuthToken(): Promise<void> {
		await storeSecret(this._context, "bluesAiCoderAccountId", undefined)
	}
	...
	async restoreRefreshTokenAndRetrieveAuthInfo(): Promise<void> {
		...
		try {
			this._bluesaicoderAuthInfo = await this._provider.provider.retrieveBluesAICoderAuthInfo(this._context)
			if (this._bluesaicoderAuthInfo) {
				this._authenticated = true
				await this.sendAuthStatusUpdate()
			} else {
