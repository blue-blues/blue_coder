...
export enum BluesAICoderErrorType {
	Auth = "auth",
	Network = "network",
	RateLimit = "rateLimit",
	Balance = "balance",
...
export class BluesAICoderError extends Error {
	readonly title = "BluesAICoderError"
	readonly _error: ErrorDetails

	// Error details per providers:
	// Blues-AI Coder: error?.error
	// Ollama: error?.cause
	// tbc
	constructor(
		raw: any,
	...
	/**
	 * Parses a stringified error into a BluesAICoderError instance.
	 */
	static parse(errorStr?: string, modelId?: string): BluesAICoderError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return BluesAICoderError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a BluesAICoderError instance.
	 * Always returns a BluesAICoderError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): BluesAICoderError {
		try {
			return new BluesAICoderError(JSON.parse(error), modelId, providerId)
		} catch {
			return new BluesAICoderError(error, modelId, providerId)
		}
	}

	public isErrorType(type: BluesAICoderErrorType): boolean {
		return BluesAICoderError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: BluesAICoderError): BluesAICoderErrorType | undefined {
		const { code, status, details } = err._error

		// Check balance error first (most specific)
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return BluesAICoderErrorType.Balance
		}

		// Check auth errors
		if (code === "ERR_BAD_REQUEST" || status === 401) {
			return BluesAICoderErrorType.Auth
		}

		// Check for auth message (only if message exists)
		const message = err.message
		if (message?.includes(BLUES_AI_CODER_ACCOUNT_AUTH_ERROR_MESSAGE)) {
			return BluesAICoderErrorType.Auth
		}
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return BluesAICoderErrorType.RateLimit
			}
		}
