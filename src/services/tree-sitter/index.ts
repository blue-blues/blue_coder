...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = bluesAICoderIgnoreController
		? bluesAICoderIgnoreController.filterPaths(filesToParse)
		: filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, bluesAICoderIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (bluesAICoderIgnoreController && !bluesAICoderIgnoreController.validateAccess(filePath)) {
		return null
	}
