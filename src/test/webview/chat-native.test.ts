...
describe("Chat Integration Tests", () => {
                            ...
                            switch (message.type) {
                                ...
                                case 'primaryButtonClick':
                                    vscode.postMessage({ 
                                        type: 'grpc_request',
                                        grpc_request: {
                                            service: 'bluesaicoder.TaskService',
                                            method: 'askResponse',
                                            message: {
	...
	it("should handle tool approval flow", async () => {
		...
		const approvalPromise = new Promise<any>((resolve) => {
			panel.webview.onDidReceiveMessage((message) => {
				if (
					...
					message.grpc_request?.service === "bluesaicoder.TaskService" &&
		...
		assert.equal(response.grpc_request.service, "bluesaicoder.TaskService")
