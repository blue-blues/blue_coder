...
describe("BluesAICoder Extension", () => {
	after(() => {
	...
	it("should verify extension ID matches package.json", async () => {
		...
		const bluesaicoderExtensionApi = vscode.extensions.getExtension(id)

		bluesaicoderExtensionApi?.id.should.equal(id)
	})

	it("should successfully execute the plus button command", async () => {
		await new Promise((resolve) => setTimeout(resolve, 400))
		await vscode.commands.executeCommand("bluesaicoder.plusButtonClicked")
	})
