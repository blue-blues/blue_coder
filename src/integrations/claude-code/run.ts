...
export async function* runClaudeCode(options: ClaudeCodeOptions): AsyncGenerator<ClaudeCodeMessage | string> {
	...
	const tempFilePath = path.join(os.tmpdir(), `bluesaicoder-system-prompt-${uniqueId}.txt`)
...
Please check our docs on how to integrate Claude Code with Cline on Windows: https://docs.bluesaicoder.bot/provider-config/claude-code#windows-setup.
Anthropic is aware of this issue and is considering a fix: https://github.com/anthropics/claude-code/issues/3411.
`,
					{ cause: err },
				)
