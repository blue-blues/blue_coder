...
class WorkspaceTracker {
	...
	async populateFilePaths() {
		// should not auto get filepaths for desktop since it would immediately show permission popup before bluesaicoder ever creates a file
		if (!this.cwd) {
			return
		}
	...
	private registerListeners() {
		 ...
		 */
		// In other words, we don't have to worry about the root workspace folder ([0]) changing since the extension will be restarted and our cwd will be updated to reflect the new workspace folder. (We don't care about non root workspace folders, since bluesaicoder will only be working within the root folder cwd)
