...
import { convertProtoToBluesAICoderMessage } from "@shared/proto-conversions/blues-ai-coder-message"
...
interface ExtensionStateContextType extends ExtensionState {
	...
	setGlobalBluesAICoderRulesToggles: (toggles: Record<string, boolean>) => void
	setLocalBluesAICoderRulesToggles: (toggles: Record<string, boolean>) => void
...
export const ExtensionStateContextProvider: React.FC<{
	...
	useEffect(() => {
		...
		partialMessageUnsubscribeRef.current = UiServiceClient.subscribeToPartialMessage(EmptyRequest.create({}), {
			onResponse: (protoMessage) => {
				try {
					...
					const partialMessage = convertProtoToBluesAICoderMessage(protoMessage)
					setState((prevState) => {
						// worth noting it will never be possible for a more up-to-date message to be sent here or in normal messages post since the presentAssistantContent function uses lock
						const lastIndex = findLastIndex(prevState.bluesAiCoderMessages, (msg) => msg.ts === partialMessage.ts)
						if (lastIndex !== -1) {
							const newBluesAiCoderMessages = [...prevState.bluesAiCoderMessages]
							newBluesAiCoderMessages[lastIndex] = partialMessage
							return { ...prevState, bluesAiCoderMessages: newBluesAiCoderMessages }
						}
						return prevState
					})
				} catch (error) {
		...
		const clientId = (window as any).bluesAiCoderClientId
		if (clientId) {
	...
	const contextValue: ExtensionStateContextType = {
		...
		showAccount,
		showAnnouncement,
		globalBluesAICoderRulesToggles: state.globalBluesAICoderRulesToggles || {},
		localBluesAICoderRulesToggles: state.localBluesAICoderRulesToggles || {},
		...
		setGlobalBluesAICoderRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				globalBluesAICoderRulesToggles: toggles,
			})),
		setLocalBluesAICoderRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				localBluesAICoderRulesToggles: toggles,
			})),
