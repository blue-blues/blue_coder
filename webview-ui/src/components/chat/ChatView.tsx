...
import { BluesAICoderApiReqInfo, BluesAICoderMessage } from "@shared/ExtensionMessage"
...
const ChatView = ({ isHidden, showAnnouncement, hideAnnouncement, showHistoryView }: ChatViewProps) => {
	...
	const task = useMemo(() => messages.at(0), [messages]) // leaving this less safe version here since if the first message is not a task, then the extension is in a bad state and needs to be debugged (see BluesAiCoder.abort)
	...
	const lastApiReqTotalTokens = useMemo(() => {
		const getTotalTokensFromApiReqMessage = (msg: BluesAICoderMessage) => {
				...
				const { tokensIn, tokensOut, cacheWrites, cacheReads }: BluesAICoderApiReqInfo = JSON.parse(msg.text)
	...
	// Use streaming hook
	const isStreaming = useIsStreaming(modifiedMessages, bluesAICoderAsk, enableButtons, primaryButtonText)
