...
interface ChatRowProps {
	...
	lastModifiedMessage?: BluesAICoderMessage
	isLast: boolean
	...
	({
		...
		const [cost, apiReqCancelReason, apiReqStreamingFailedMessage, retryStatus] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: BluesAICoderApiReqInfo = JSON.parse(message.text)
			...
			switch (type) {
				...
				case "mistake_limit_reached":
					return [
						...
						<span style={{ color: errorColor, fontWeight: "bold" }}>Blues-AI Coder is having trouble...</span>,
					]
				...
				case "command":
					return [
						...
						<span style={{ color: normalColor, fontWeight: "bold" }}>Blues-AI Coder wants to execute this command:</span>,
					]
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as BluesAICoderAskUseMcpServer
					return [
						...
						<span
							...
							Blues-AI Coder wants to {mcpServerUse.type === "use_mcp_tool" ? "use a tool" : "access a resource"} on the{" "}
				...
				case "followup":
					return [
						...
						<span style={{ color: normalColor, fontWeight: "bold" }}>Blues-AI Coder has a question:</span>,
					]
				default:
					return [null, null]
			}
		...
		const tool = useMemo(() => {
			if (message.ask === "tool" || message.say === "tool") {
				return JSON.parse(message.text || "{}") as BluesAICoderSayTool
			}
			return null
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>Blues-AI Coder wants to edit this file:</span>
							</div>
							<CodeAccordian
					...
					return (
						<>
							<div style={headerStyle}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span style={{ fontWeight: "bold" }}>Blues-AI Coder wants to create a new file:</span>
							</div>
							<CodeAccordian
								isLoading={message.partial}
								code={tool.content!}
						...
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>
									{/* {message.type === "ask" ? "" : "Blues-AI Coder read this file:"} */}
									Blues-AI Coder wants to read this file:
								</span>
							</div>
							<div
								style={{
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Blues-AI Coder wants to view the top level files in this directory:"
										: "Blues-AI Coder viewed the top level files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Blues-AI Coder wants to recursively view all files in this directory:"
										: "Blues-AI Coder recursively viewed all files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Blues-AI Coder wants to view source code definition names used in this directory:"
										: "Blues-AI Coder viewed source code definition names used in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<>
							<div style={headerStyle}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This is outside of your workspace")}
								<span style={{ fontWeight: "bold" }}>
									Blues-AI Coder wants to search this directory for <code>{tool.regex}</code>:
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Blues-AI Coder wants to fetch content from this URL:"
										: "Blues-AI Coder fetched content from this URL:"}
								</span>
							</div>
							<div
								style={{
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as BluesAICoderAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					...
					case "user_feedback_diff":
						const tool = JSON.parse(message.text || "{}") as BluesAICoderSayTool
						return (
							<div
								style={{
									marginTop: -10,
					...
					case "blues_ai_coder_ignore_error":
						return <ErrorRow message={message} errorType="blues_ai_coder_ignore_error" />
							...
							<>
								<div
									...
									<div style={{ color: "var(--vscode-foreground)", opacity: 0.8 }}>
										Blues-AI Coder may have trouble viewing the command's output. Please update VSCode (
										<code>CMD/CTRL + Shift + P</code> → "Update") and make sure you're using a supported
										shell: zsh, bash, fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal:
										Select Default Profile").{" "}
										<a
											href="https://github.com/Exaf-AI/blues-ai-coder/wiki/Troubleshooting-%E2%80%90-Shell-Integration-Unavailable"
											style={{
												color: "inherit",
												textDecoration: "underline",
											}}>
			...
			case "ask":
				switch (message.ask) {
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as BluesAICoderAskQuestion
							...
							<>
								<div style={headerStyle}>
									...
									<span style={{ color: normalColor, fontWeight: "bold" }}>
										Blues-AI Coder wants to start a new task:
									</span>
								</div>
						...
						return (
							<>
								<div style={headerStyle}>
									...
									<span style={{ color: normalColor, fontWeight: "bold" }}>
										Blues-AI Coder wants to condense your conversation:
									</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</>
						...
						return (
							<>
								<div style={headerStyle}>
									...
									<span style={{ color: normalColor, fontWeight: "bold" }}>
										Blues-AI Coder wants to create a Github issue:
									</span>
								</div>
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as BluesAICoderPlanModeResponse
