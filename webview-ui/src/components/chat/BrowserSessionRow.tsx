	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: BluesAICoderMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: BluesAICoderMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: BluesAICoderMessage[] = []
		let nextActionMessages: BluesAICoderMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					<>{isAutoApproved ? "Blues AI Coder is using the browser:" : "Blues AI Coder wants to use the browser:"}</>
				</span>
			</div>
			<div
				style={{
