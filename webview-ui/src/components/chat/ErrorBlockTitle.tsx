...
export const ErrorBlockTitle = ({
	...
	const title = (() => {
		...
		if (apiReqCancelReason === "user_cancelled") {
			...
			const bluesAiCoderError = BluesAICoderError.parse(apiRequestFailedMessage)
			const titleText = bluesAiCoderError?.isErrorType(BluesAICoderErrorType.Balance) ? "Credit Limit Reached" : "API Request Failed"
