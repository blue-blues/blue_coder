...
import {
	...
	RefreshedRules,
	ToggleBluesAICoderRuleRequest,
	ToggleBluesAICoderRules,
...
import BluesAiCoderRulesToggleList from "./BluesAiCoderRulesToggleList"

const BluesAiCoderRulesToggleModal: React.FC = () => {
	const {
		globalBluesAICoderRulesToggles = {},
		localBluesAICoderRulesToggles = {},
		localCursorRulesToggles = {},
		localWindsurfRulesToggles = {},
		localWorkflowToggles = {},
		globalWorkflowToggles = {},
		setGlobalBluesAICoderRulesToggles,
		setLocalBluesAICoderRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalBluesAiCoderRulesToggles?.toggles) {
						setGlobalBluesAICoderRulesToggles(response.globalBluesAiCoderRulesToggles.toggles)
					}
					if (response.localBluesAiCoderRulesToggles?.toggles) {
						setLocalBluesAICoderRulesToggles(response.localBluesAiCoderRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalBluesAICoderRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localBluesAICoderRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleBluesAICoderRule(
			ToggleBluesAICoderRuleRequest.create({
				isGlobal,
				rulePath,
				enabled,
			}),
		)
			.then((response: ToggleBluesAICoderRules) => {
			...
			.catch((error: Error) => {
				console.error("Error toggling Blues-AI Coder rule:", error)
			})
	}
	...
	const toggleCursorRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleCursorRule(
			...
			.then((response: BluesAiCoderRulesToggles) => {
				// Update the local state with the response
				if (response.toggles) {
	...
	return (
		<div ref={modalRef}>
			<div ref={buttonRef} className="inline-flex min-w-0 max-w-full">
				<Tooltip tipText="Manage Blues-AI Coder Rules & Workflows" visible={isVisible ? false : undefined}>
					<VSCodeButton
						appearance="icon"
						aria-label="Blues-AI Coder Rules"
			...
			{isVisible && (
				<div
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rules" ? (
							<p>
								Rules allow you to provide Blues-AI Coder with system-level guidance. Think of them as a persistent way to
								include context and preferences for your projects or globally for every conversation.{" "}
								<VSCodeLink
									href="https://docs.blues-ai-coder.com/features/blues-ai-coder-rules"
							...
							<p>
								Workflows allow you to define a series of steps to guide Blues-AI Coder through a repetitive set of tasks,
								...
								in the chat.{" "}
								<VSCodeLink
									href="https://docs.blues-ai-coder.com/features/slash-commands/workflows"
									style={{ display: "inline" }}
									className="text-xs">
									Docs
								</VSCodeLink>
...
export default BluesAiCoderRulesToggleModal
