...
import { UsageTransaction as BluesAICoderAccountUsageTransaction, PaymentTransaction } from "@shared/BluesAiCoderAccount"
...
import BluesAICoderLogoWhite from "../../assets/BluesAICoderLogoWhite.tsx"
...
const BLUES_AI_CODER_APP_URL = "https://app.blues-ai-coder.com"

export const BluesAICoderAccountView = () => {
	const { bluesAiCoderUser, handleSignIn, handleSignOut } = useBluesAICoderAuth()
	const { userInfo, apiConfiguration } = useExtensionState()

	const user = apiConfiguration?.bluesAiCoderAccountId ? bluesAiCoderUser || userInfo : undefined
	...
	const [usageData, setUsageData] = useState<BluesAICoderAccountUsageTransaction[]>([])
	const [paymentsData, setPaymentsData] = useState<PaymentTransaction[]>([])
	const [lastFetchTime, setLastFetchTime] = useState<number>(Date.now())

	const bluesAICoderUris = useMemo(() => {
		const base = new URL(bluesAiCoderUser?.appBaseUrl || BLUES_AI_CODER_APP_URL)
	...
	}, [bluesAiCoderUser?.appBaseUrl, activeOrganization])
	...
	const fetchCreditBalance = useCallback(async () => {
		try {
			...
			const bluesAICoderUsage = convertProtoUsageTransactions(response.usageTransactions)
			setUsageData(bluesAICoderUsage || [])
	...
	useEffect(() => {
		...
		loadData()
	}, [activeOrganization?.organizationId, bluesAiCoderUser?.uid])
	...
	}, [bluesAiCoderUser?.uid, getUserOrganizations, fetchCreditBalance])
		...
		<div className="h-full flex flex-col">
			{user ? (
				<div className="flex flex-col pr-3 h-full">
					...
					<div className="w-full flex gap-2 flex-col min-[225px]:flex-row">
						<div className="w-full min-[225px]:w-1/2">
							<VSCodeButtonLink href={bluesAICoderUris.dashboard.href} appearance="primary" className="w-full">
								Dashboard
							</VSCodeButtonLink>
						</div>
					...
					<div
						...
						<div className="w-full">
							<VSCodeButtonLink href={bluesAICoderUris.credits.href} className="w-full">
								Add Credits
							</VSCodeButtonLink>
						</div>
					</div>
				...
				<div className="flex flex-col items-center pr-3">
					<BluesAICoderLogoWhite className="size-16 mb-4" />
					...
					<p className="text-[var(--vscode-descriptionForeground)] text-xs text-center m-0">
						By continuing, you agree to the <VSCodeLink href="https://blues-ai-coder.com/tos">Terms of Service</VSCodeLink> and{" "}
						<VSCodeLink href="https://blues-ai-coder.com/privacy">Privacy Policy.</VSCodeLink>
					</p>
				</div>
			)}
		</div>
...
/**
 * Converts a protobuf UsageTransaction to a BluesAICoderAccount UsageTransaction
 * by adding the missing id and metadata fields
 */
function convertProtoUsageTransaction(protoTransaction: ProtoUsageTransaction): BluesAICoderAccountUsageTransaction {
	return {
		...protoTransaction,
...
/**
 * Converts an array of protobuf UsageTransactions to BluesAICoderAccount UsageTransactions
 */
function convertProtoUsageTransactions(protoTransactions: ProtoUsageTransaction[]): BluesAICoderAccountUsageTransaction[] {
